...
//     {
//         Logger.Log("[NavmeshSurface] Loading scene: " + sceneName);
//         if (sceneName == "SurfaceScene")
//         {
//             Logger.Log("[NavmeshSurface] Loading surface scene");
...
public class PlayerAwakePatch
{
    ...
    static void Postfix()
    {
        Logger.Log("[NavmeshSurface] Player Awake. current scene: " + SceneManager.GetActiveScene().name);
        if (SceneManager.GetActiveScene().name == "SurfaceScene" && _myInstance == null)
        {
            Logger.Log("[NavmeshSurface] Instantiating surface navmesh");
